    ...
    <main class="op-converter-wrap">
        ...
        <div class="container" v-if="!resultReady && files.length === 0">
            <section class="op-converter op-converter-start">
                ...
                <div class="op-upload-area" @dragover.prevent="dropHover = true" @dragleave.prevent="dropHover = false" @drop.prevent="onDrop" :class="{ hover: dropHover }">
        ...
        <div class="container-fluid op-work-layout" v-else-if="!resultReady">
            <section class="op-work-left" @dragover.prevent="dropHover = true" @dragleave.prevent="dropHover = false" @drop.prevent="onDrop" :class="{ hover: dropHover }">
                <button type="button" class="op-floating-add" @click="triggerFileSelect" :title="config.i18n.workspace.add_more_files">
                    <span class="op-floating-count">@{{ files.length }}</span>
                    <i class="bi bi-plus-lg"></i>
                </button>

                <button type="button" class="op-floating-sort" v-if="!isSplitTool && files.length > 1" @click="sortFilesByName" :title="config.i18n.workspace.sort_files">
                ...
                <div v-else class="file-grid op-file-grid-work" ref="sortableGrid">
                    <article v-for="(file, index) in files" :key="file.id" class="file-card" :data-file-id="file.id">
                        ...
                        <div class="preview-box">
                            <img v-if="file.preview" :src="file.preview" :alt="file.name" :style="{ transform: `rotate(${file.rotation}deg)` }">
                            <div v-else class="preview-placeholder">
                                <i class="bi bi-file-earmark-pdf"></i>
            ...
            <aside class="op-work-right">
                ...
                <p v-if="feedbackMessage" class="op-feedback op-feedback-side" :class="`is-${feedbackType}`">@{{ feedbackMessage }}</p>

                <div class="op-settings op-settings-side" v-if="isSplitTool">
                    <div class="op-split-tabs">
                        <button type="button" class="op-split-tab" :class="{ active: splitViewMode === 'range' }" @click="setSplitViewMode('range')">
                            <i class="bi bi-braces"></i>
                            <span>@{{ config.i18n.workspace.split_mode_range }}</span>
                        </button>
                        <button type="button" class="op-split-tab" :class="{ active: splitViewMode === 'pages' }" @click="setSplitViewMode('pages')">
                    ...
                    <div class="op-split-block">
                        ...
                        <div class="op-split-actions">
                            <button type="button" class="op-split-action" :class="{ active: splitExtractMode === 'all' }" @click="setSplitExtractMode('all')">
                                @{{ config.i18n.workspace.split_extract_all }}
                            </button>
                            <button type="button" class="op-split-action" :class="{ active: splitExtractMode === 'selected' }" @click="setSplitExtractMode('selected')">
                    ...
                    <div class="form-check mt-3">
                        <input class="form-check-input" type="checkbox" id="splitMergeExtracted" v-model="splitMergeExtracted">
                        <label class="form-check-label" for="splitMergeExtracted">@{{ config.i18n.workspace.split_merge_into_one }}</label>
                    </div>
                </div>

                <div class="op-settings op-settings-side" v-else-if="selectedTool && selectedTool.key === 'jpg_to_pdf'">
                    <div class="mb-2">
                ...
                <div class="op-work-cta">
                    <button type="button" class="btn op-primary-btn op-convert-btn op-convert-panel-btn" @click="convert" :disabled="busy || !canConvert">
        ...
        <div class="container op-result-stage" v-else>
            ...
            <div class="op-result-actions">
                <button type="button" class="op-result-back" @click="resetWorkspace" :title="config.i18n.workspace.back_to_editor">
