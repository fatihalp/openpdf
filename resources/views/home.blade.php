...
<div id="openpdf-app" class="app-shell" v-cloak>
    <header class="site-header sticky-top">
        <nav class="navbar navbar-expand-lg">
            <div class="container">
                <a class="navbar-brand fw-bold" href="#">@{{ config.domain }}</a>
                ...
                <div class="collapse navbar-collapse" id="mainNav">
                    <ul class="navbar-nav ms-auto me-3">
                        ...
                        <li class="nav-item"><a class="nav-link" href="#policy">@{{ config.i18n.nav.limits }}</a></li>
                    ...
                    <form method="post" action="{{ route('locale.switch') }}" class="d-flex align-items-center gap-2">
                        @csrf
                        <select name="locale" class="form-select form-select-sm" style="min-width: 130px;" onchange="this.form.submit()">
                            @foreach ($i18n = trans('openpdf.locales') as $code => $label)
                                <option value="{{ $code }}" @selected(app()->getLocale() === $code)>{{ $label }}</option>
                            @endforeach
                        </select>
                        <span class="badge" :class="auth.logged_in ? 'text-bg-success' : 'text-bg-secondary'">@{{ auth.logged_in ? auth.name : config.i18n.auth.visitor }}</span>
                        <button v-if="auth.logged_in" type="button" class="btn btn-sm btn-outline-secondary" @click="logout">@{{ config.i18n.auth.logout }}</button>
                    </form>
                </div>
            </div>
        </nav>
                ...
                <div class="hero-wrap">
                    ...
                    <div class="hero-auth card border-0 shadow-sm">
                        <div class="card-body">
                            ...
                            <small class="text-muted" v-if="!config.googleClientId">Google Client ID tanimli degil.</small>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="workspace-grid">
                        ...
                        <div class="card-body">
                            ...
                            <div v-if="selectedTool && selectedTool.key === 'jpg_to_pdf'" class="file-grid">
                                <article
                                    ...
                                    class="file-card"
                                    draggable="true"
                                    @dragstart="startDrag(file.id)"
                                    @dragover.prevent
                                    @drop.prevent="dropOn(file.id)"
                                >
                                    <div class="actions">
                                        <button type="button" class="btn btn-sm btn-danger rounded-circle" @click="rotate(index)">↻</button>
                                        <button type="button" class="btn btn-sm btn-light rounded-circle" @click="remove(index)">✕</button>
                                    </div>
                                    <div class="preview">
                            ...
                            <div v-else class="list-group mt-3">
                                <div v-for="(file, index) in files" :key="file.id" class="list-group-item d-flex justify-content-between align-items-center">
                                    <div>
                                        <strong>@{{ file.name }}</strong>
                                        <small class="d-block text-muted">@{{ formatBytes(file.size) }}</small>
                                    </div>
                                    <button type="button" class="btn btn-sm btn-outline-danger" @click="remove(index)">Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <aside class="workspace-side card border-0 shadow-sm">
                        <div class="card-body">
                            ...
                            <button type="button" class="btn btn-danger w-100" @click="convert" :disabled="busy || files.length === 0">
    ...
    <footer id="about" class="site-footer">
        <div class="container py-4">
            <h3>@{{ config.domain }}</h3>
            <p class="mb-1">@{{ config.i18n.footer.about }}</p>
            <p class="mb-1">@{{ config.i18n.footer.domain }}</p>
            <p class="mb-0">@{{ config.i18n.footer.model }}</p>
        </div>
    </footer>
</div>
